...
const createGroupInvite = async (req, res, next) => {
  try {
    ...
    const shareMessage = `${authUser.fullName || 'A member'} has invited you.`;
...
const scanGuestInvite = async (req, res, next) => {
  try {
    ...
    const qrType = payload.type || null;
    ...
    const guestEntryRequest = await GuestEntryRequest.create({
      ...
      guestPhoneNumber: arrivingGuest?.phoneNumber || null,
      guestPhoneDigits: isTenDigitPhone(arrivingGuest?.phoneNumber) ? normalizePhoneDigits(arrivingGuest.phoneNumber) : null,
      guestImageUrl: null,
      visitorType: 'guest',
...
const updateGuestInviteEntryDetails = async (req, res, next) => {
  try {
    ...
    const guestId = getLastBodyValue(payload.guestId);
    const vehicleNumber = getLastBodyValue(payload.vehicleNumber);
    const accompanyingCount = getLastBodyValue(payload.accompanyingCount);
    const imageUrl = getLastBodyValue(
      payload.image !== undefined
        ? payload.image
        : payload.imageUrl
    );
    const guestName = getLastBodyValue(payload.guestName);
