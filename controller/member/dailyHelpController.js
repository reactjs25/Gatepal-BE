...
const editDailyHelpProfile = async (req, res, next) => {
  try {
    ...
    const category = getLastBodyValue(body.category);
    const name = getLastBodyValue(body.name);
    const phoneNumber = getLastBodyValue(body.phoneNumber);
    const countryCode = getLastBodyValue(body.countryCode);
    const imageRaw =
