...
const validateUploadPayload = (payload = {}) => {
    const yearRaw = getLastBodyValue(payload.year);
    ...
    const month = toCanonicalMonth(getLastBodyValue(payload.month));
    ...
    const amount = parseCurrencyAmount(getLastBodyValue(payload.amount));
    if (amount === null) {
        throw createHttpError('amount is required and must be a valid number.', 400);
    }

    const transactionDate = toDateOrNull(getLastBodyValue(payload.transactionDate));
    if (!transactionDate) {
    ...
    const proofImageInput = normalizeString(getLastBodyValue(proofImageRaw));
    if (!proofImageInput) {
