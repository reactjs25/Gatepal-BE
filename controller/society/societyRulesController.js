...
const validateSocietyRulePayload = async (payload = {}, options = {}) => {
  ...
  const categoryKeyRaw = getLastBodyValue(payload.categoryKey);
  const contentRaw = getLastBodyValue(payload.contentHtml);
  const photoRaw =
...
const getSocietyRuleById = async (req, res, next) => {
  try {
    ...
    const ruleId = normalizeString(
      getLastBodyValue(((req.body || {}).ruleId)) || ((req.params && req.params.ruleId) || '')
    );
    if (!ruleId) {
