...
const addSocietyDailyHelp = async (req, res, next) => {
  try {
    ...
    const complianceConfirmed = getLastBodyValue(payload.complianceConfirmed);
    ...
    if (imageRaw !== undefined) {
      try {
        formattedImage = await normalizeImageInputToStorageUrl({
          value: getLastBodyValue(imageRaw),
          fieldLabel: 'Image',
...
const rejectDailyHelp = async (req, res, next) => {
  try {
    ...
    const rejectPayload = req.body || {};
    const rejectReason = getLastBodyValue(rejectPayload.rejectReason);
    const description = getLastBodyValue(rejectPayload.description);
    const reasonCode = getLastBodyValue(rejectPayload.reasonCode);
    const reasonText = getLastBodyValue(rejectPayload.reasonText);
    const unitId = getLastBodyValue(rejectPayload.unitId);
    const unitNumber = getLastBodyValue(rejectPayload.unitNumber);
    const name = getLastBodyValue(rejectPayload.name);
    const category = getLastBodyValue(rejectPayload.category);
    const phoneNumber = getLastBodyValue(rejectPayload.phoneNumber);
    const imageUrl = getLastBodyValue(
      rejectPayload.image !== undefined
        ? rejectPayload.image
        : rejectPayload.imageUrl
    );
...
const editSocietyDailyHelpProfile = async (req, res, next) => {
  try {
    ...
    if (imageRaw !== undefined && typeof getLastBodyValue(imageRaw) !== 'string') {
